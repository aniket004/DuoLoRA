...
from ziplora_pytorch.utils import (
    ...
    ziplora_compute_mergers_rank_loss_cycle,
)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["gaussian", "kaiming_uniform", "boolean"],
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help="content_init_thresh",
    )    
    parser.add_argument(
        "--style_init_thresh",
        type=float,
        default=0.5,
        help="style_init_thresh",
    )   
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="an appropriate multiplier for the rank loss term",
    )   
    parser.add_argument(
        "--cycle_lambda",
        type=float,
        default=0.01,
        help="an appropriate multiplier for the cycle loss term",
    )   
    parser.add_argument(
        "--rev_cycle_lambda",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="an appropriate multiplier for the nuclear_norm_coeff loss term",
    )    
    parser.add_argument(
        "--sparsity_coeff",
        type=float,
        default=1.0,
        help="an appropriate multiplier for the sparsity_coeff loss term",
    )    
    parser.add_argument(
        "--text_encoder_lr",
        type=float,
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_prompt_3 = instance_prompt_3
    ...
    def __getitem__(self, index):
        ...
        example["instance_prompt_3"] = self.instance_prompt_3
        return example
...
def collate_fn(examples):
    ...
    prompts_3 = [example["instance_prompt_3"] for example in examples]
...
def main(args):
    ...
    for attn_processor_name, attn_processor in unet.attn_processors.items():
        ...
        merged_lora_weights_dict, split_dict_up, split_dict_down  = merge_lora_weights(lora_weights, attn_name)
        merged_lora_weights_dict_2, split_dict_2_up, split_dict_2_down = merge_lora_weights(lora_weights_2, attn_name)

        #breakpoint()
        
        kwargs = {
            "state_dict_up": split_dict_up,
            "state_dict_down": split_dict_down,
            "state_dict_2_up": split_dict_2_up,
            "state_dict_2_down": split_dict_2_down,
        }
        ...
        if  '.'.join(attn_processor_name.split('.')[:2]) in ['up_blocks.2', 'down_blocks.2', 'mid_blocks.2' ]: # 16x16
            # R(content) > R(style)
            # initialize more content with 1's and style with zero's
            ## add 

            if args.merger_init_method=='gaussian':
                # Generate a random vector between (0,1)
                random_vector = torch.randn((64, 1))
                random_vector = (random_vector - random_vector.min()) / (random_vector.max() - random_vector.min())
            elif args.merger_init_method=='kaiming_uniform':
                random_vector = torch.empty((64, 1))
                torch.nn.init.kaiming_uniform_(random_vector, a=0)
            else:
                random_vector = torch.ones((64, 1))

            # Generate binary vectors based on threshold
            binary_vector_content = random_vector.float().cuda() * (random_vector > args.style_init_thresh).float().cuda()
            binary_vector_style = random_vector.float().cuda() * (random_vector > args.content_init_thresh).float().cuda()

            # Ensure no gradients are required and clone to avoid any issues with graph retention
            binary_vector_content = binary_vector_content.clone().detach().requires_grad_(False)
            binary_vector_style = binary_vector_style.clone().detach().requires_grad_(False)

            #breakpoint()
            #print(f"attn_module.to_q.lora_layer: {attn_module.to_q.lora_layer}")
            ...
            attn_module.to_q.set_lora_layer(
                initialize_ziplora_layer(
                    ...
                    init_merger_value=binary_vector_content,
                    init_merger_value_2=binary_vector_style,
                    **kwargs,
                )
            )
            ...
            #breakpoint()
        elif '.'.join(attn_processor_name.split('.')[:2]) in ['up_blocks.1', 'down_blocks.1']: # 32x32
            # R(style) > R(content)
            # initilize more styles with 1's 
            # Generate a random vector between (0,1)
            # random_vector = torch.randn((64, 1))
            # random_vector = (random_vector - random_vector.min()) / (random_vector.max() - random_vector.min())
            ...
            # Generate binary vectors based on threshold
            binary_vector_content = random_vector.float().cuda() * (random_vector > args.content_init_thresh).float().cuda()
            binary_vector_style = random_vector.float().cuda() * (random_vector > args.style_init_thresh).float().cuda()
            ...
            # Generate binary vectors based on threshold
            binary_vector_content = random_vector.float().cuda() * (torch.ones((64,1))).float().cuda()
            binary_vector_style = random_vector.float().cuda() * (torch.ones((64,1))).float().cuda()
    ...
    train_dataset = DreamBoothDataset(
        ...
        instance_prompt_3=args.instance_prompt_3,
    ...
    if not args.train_text_encoder:
        ...
        (
            instance_prompt_hidden_states_3,
            instance_pooled_prompt_embeds_3,
        ) = compute_text_embeddings(args.instance_prompt_3, text_encoders, tokenizers)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                unet_added_conditions_3 = {
                    "time_ids": unet_added_conditions["time_ids"],
                    "text_embeds": unet_add_text_embeds_3.repeat(bsz, 1),
                }
                prompt_embeds_input_3 = prompt_embeds_3.repeat(bsz, 1, 1)
                ...
                ## reverse cycle
                # 7. concepts weights + style
                ziplora_set_forward_type(unet, type="weight_1")
                with torch.no_grad():
                    model_pred_sc = unet(
                        model_inputs[1],  # input I_s
                        timesteps,
                        prompt_embeds_input_3,
                        added_cond_kwargs=unet_added_conditions_3,
                    ).sample
                ...
                #breakpoint()
                alpha_prod_t = noise_scheduler.alphas_cumprod[timesteps.cpu()].to(model_inputs[0].device)
                beta_prod_t = (1 - alpha_prod_t).to(model_inputs[0].device)
                x_0_pred_cs = (model_inputs[0] - beta_prod_t ** (0.5) * model_pred_cs) / alpha_prod_t ** (0.5)

                x0_noise_pred_cs = noise_scheduler.add_noise(x_0_pred_cs, noise, timesteps)

                # get zt of I_s
                x_0_pred_sc = (model_inputs[1] - beta_prod_t ** (0.5) * model_pred_sc) / alpha_prod_t ** (0.5)
                x_0_noise_pred_sc = noise_scheduler.add_noise(x_0_pred_sc, noise, timesteps)
 
                # 6. concept weights + style + concept
                ziplora_set_forward_type(unet, type="weight_2")
                with torch.no_grad():
                    model_pred_csc = unet(
                        x0_noise_pred_cs,
                ...
                # reverse cycle 
                # 8. concepts weights + style
                ziplora_set_forward_type(unet, type="weight_1")
                with torch.no_grad():
                    model_pred_scs = unet(
                        x_0_noise_pred_sc,  # input I_s
                        timesteps,
                        prompt_embeds_input_2,
                        added_cond_kwargs=unet_added_conditions_2,
                    ).sample
                ...
                loss_4 = args.cycle_lambda * F.mse_loss(
                    model_pred_mc.float(), model_pred_csc.float(), reduction="mean"
                )
                #breakpoint()
                loss_5 = args.rank_lambda * ziplora_compute_mergers_rank_loss_cycle(unet, args.nuclear_norm_coeff, args.sparsity_coeff)

                loss_6 = args.rev_cycle_lambda * F.mse_loss(
                    model_pred_ms.float(), model_pred_scs.float(), reduction="mean"
                )
            ...
            logs = {
                ...
                "loss_4": loss_4.detach().item(),
                "loss_5": loss_5.detach().item(),
                "loss_6": loss_6.detach().item(),
